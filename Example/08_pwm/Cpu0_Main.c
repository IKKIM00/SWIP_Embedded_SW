...
__interrupt(0x0B)__vector_table(0)void CCU60_T12_ISR (void)
{
...
int core0_main (void)
{
    ...
    while (1)
    {
        ...
        if (adcResult >= 3096)
        {
            ...
            GTM_TOM0_CH1_SR1.U = adcResult*2;
        }
...
void initVADC (void)
{
    ...
    while ((VADC_CLC.U & (1 << DISS_BIT_LSB_IDX)) != 0)
        ; // wait until VADC module clock enabled
    ...
    // VADC Groupt 4 Channel 7 configuration
    VADC_G4_CHCTR7.U |= 0x1 << RESPOS_BIT_LSB_IDX; // result right-aligned
    VADC_G4_CHCTR7.U &= ~(0x1 << RESREG_BIT_LSB_IDX); // store result @ Rsult Register G4RES0
    VADC_G4_CHCTR7.U &= ~(0x3 << ICLSEL_BIT_LSB_IDX); // Clas 0
...
void initCCU60 (void)
{
    ...
    while ((CCU60_CLC.U & (1 << DISS_BIT_LSB_IDX)) != 0)
        ; // wait until CCU60 module clock enabled
...
void initGTM (void)
{
    ...
    while ((GTM_CLC.U & (1 << DISS_BIT_LSB_IDX)) != 0)
        ; // wait until GTM module clock enabled
    ...
    GTM_TOM0_CH1_SR0.U = 12500 - 1; // PWM freq. = 12500 / 6250 kHz = 2ms ==> CM0 +1 / Freq.of CMU_FXCLK1
    GTM_TOM0_CH1_SR1.U = 1250 - 1 ; // duty cycle = 1250 / 12500 = 10% ==> C


    GTM_TOUTSEL6.U &= ~(0x3 << SEL7_BIT_LSB_IDX);   // TOUT103 --> TOM0 channel 1
                                                    // 103 = 16 * 6 + 7 // TOUTSEL6 -> TOUT96 ~ TOUT103
